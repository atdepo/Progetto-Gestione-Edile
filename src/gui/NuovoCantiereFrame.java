			...
			public void actionPerformed(ActionEvent e) {
				if(!valore.getText().isEmpty()&&Double.parseDouble(valore.getText())>500000.0D) {
					...
					if(!Dipendente.isDirigente(d)) {
						JOptionPane.showMessageDialog(mainPanel,"Valore cantiere >500000, assegna un Dirigente","ERROR",JOptionPane.ERROR_MESSAGE);
						return;
					}
				}
				 if(lat.getText().isEmpty()||lon.getText().isEmpty()||est.getText().isEmpty()||Double.parseDouble(valore.getText())<=0||Double.parseDouble(est.getText())<0) {
					 JOptionPane.showMessageDialog(mainPanel,"Inserisci dei dati corretti","ERROR",JOptionPane.ERROR_MESSAGE);
					 return;
				 }
				 
				repartoOperativo.apriCantiere(Double.parseDouble(valore.getText()), Double.parseDouble(lat.getText()), Double.parseDouble(lon.getText()), Double.parseDouble(est.getText()),scelta.get(resp.getSelectedIndex()));
